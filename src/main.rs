...
/// Example: 8 pixels * 20 = 160px magnifier
...
/// Allows zoom to be adjusted via scroll wheel
...
fn capture_zoom_area(x: f64, y: f64, size: f64) -> Option<CGImage> {
    ...
    // Get the main display for coordinate conversion
    ...
    // Calculate the capture rectangle centered on the cursor
...
fn main() {
    ...
    unsafe {
        ...
        // (Borderless windows normally can't receive keyboard events)
...
/// Registers a custom NSView subclass called "MouseBlockerView"
...
fn register_view_class() -> &'static objc::runtime::Class {
    ...
    // Create a new class declaration with name "MouseBlockerView"
...
extern "C" fn key_down(_this: &Object, _cmd: Sel, event: id) {
    unsafe {
        ...
        let (dx, dy): (f64, f64) = match key_code {
            ...
            125 => (0.0, -move_amount),   // Down arrow (screen Y increases downward)
        ...
        if dx != 0.0 || dy != 0.0 {
            ...
            let new_y = current_pos.y - dy;  // CG Y axis is inverted vs Cocoa
        ...
        if let Ok(state) = MOUSE_STATE.lock() {
            if let Some(ref info) = *state {
                ...
                if let Some(cg_image) = capture_zoom_area(info.screen_x, info.screen_y, CAPTURED_PIXELS) {
                    ...
                    // This is a bit hacky but necessary for the FFI bridge
                    let cg_image_ptr = {
                    ...
                    // Reticle is slightly smaller than one pixel
                    ...
                    // Use a specific bold font by name
                    let font_cls = class!(NSFont);
                    
                    // Try "Helvetica-Bold" which is always available on macOS
                    let font_name = NSString::alloc(nil);
                    let font_name = NSString::init_str(font_name, "Helvetica-Bold");
                    let font: id = msg_send![font_cls, fontWithName: font_name size: HEX_FONT_SIZE];
                    
                    // Fallback to system font if Helvetica-Bold not found
                    let font: id = if font != nil { 
                        font 
                    } else { 
                        msg_send![font_cls, systemFontOfSize: HEX_FONT_SIZE]
                    };
                    ...
                    let text_color: id = if luminance > 0.5 {
                        msg_send![cls, blackColor]  // Dark text on light background
                    } else {
                        msg_send![cls, whiteColor]  // Light text on dark background
                    };
                    ...
                    // Create NSDictionary with attributes
